...
import { isEmpty, removeTopic } from "../../src/utils/util";
import { ResponseError } from "../../src/utils/types";

export namespace Illusive {
    export const illusi_icon = require('./assets/illusi_icon.png');
    export const illusi_dark_icon = require('./assets/illusi_dark_icon.png');
    export const imported_thumbnail = require('./assets/imported_thumbnail.png');
    ...
    export async function convert_track(track: Track, to_music_service: MusicServiceType): Promise<Track|ResponseError>{
        ...
        const tartist = track.artists[0].toLowerCase();
        for(let i = 0; i < search_tracks.tracks.length; i++){
            ...
            const cartist = ctrack.artists[0].toLowerCase();
            if(ctitle.includes(ttitle)) current.value += 30;
            if(ctitle.includes(ttitle) && ctitle.includes(removeTopic(tartist))) current.value += 40;
            if(ctitle.includes(ttitle) && cartist.includes(removeTopic(tartist))) current.value += 55;
            if(cartist.includes(removeTopic(tartist))) current.value += 15;
    ...
    export async function get_download_url(track: Track): Promise<ExportTrack|ResponseError>{
        if(!isEmpty(track.youtube_id))
            return await music_service.get("YouTube")!.download_from_id!(track.youtube_id!);
        else if(!isEmpty(track.soundcloud_permalink))
            return await music_service.get("SoundCloud")!.download_from_id!(track.soundcloud_permalink!);
        const new_track_data = await convert_track(track, "YouTube");
        if("error" in new_track_data) return new_track_data;
        const youtube_dl_response = await music_service.get("YouTube")!.download_from_id!(new_track_data.youtube_id!);
    ...
    export async function get_best_track_artwork(track: Track): Promise<Artwork> {
        if(!isEmpty(track.imported_id))
            return imported_thumbnail;
        if(!isEmpty(track.thumbnail_uri))
        ...
        if(!isEmpty(track.service_thumbnail))
            return {"uri": track.service_thumbnail!, "cache": 'force-cache'};
    ...
    export async function get_suggestions(query: string){ return await Origin.Google.getSuggestions(query); }

    export async function get_track_lryics(track: Track){
        const search_response = await Origin.Genius.search(track.title);
        if("error" in search_response) return search_response;
        const lyrics_response = await Origin.Genius.getLyrics(search_response);
