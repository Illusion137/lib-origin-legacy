...
async function download_track(track: Track, progress_updater?: SetState, start_download?: SetState, set_finished_downloaded?: SetState): Promise<DownloadTrackResult> {
    ...
    GLOBALS.downloading.push({ 'uid': track.uid, 'progress': 0, 'progress_updater': progress_updater, 'execution_id': 0 });
    ...
    wait_for(() => in_download_range(track.uid, download_queue_max_length))
        .then(async () => {
            const download_uri = await Illusive.get_download_url(track);
