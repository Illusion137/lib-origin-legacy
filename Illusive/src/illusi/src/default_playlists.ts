...
export const default_playlists: DefaultPlaylist[] = [
    ...
    { "name": "Downloaded", "track_function": (async() => {
        await SQLActions.fetch_track_data();
		const default_playlist_max_size = Prefs.get_pref('default_playlist_max_size');
        const tracks = [...GLOBALS.global_var.sql_tracks].reverse().filter(track => !isEmpty(track.media_uri) && isEmpty(track.imported_id)).slice(0, default_playlist_max_size);
        return tracks;
    }) },
    { "name": "Imported", "track_function": (async() => {
        await SQLActions.fetch_track_data();
		const default_playlist_max_size = Prefs.get_pref('default_playlist_max_size');
        const tracks = [...GLOBALS.global_var.sql_tracks].reverse().filter(track => !isEmpty(track.imported_id)).slice(0, default_playlist_max_size);
        return tracks;
    }) },
